...
interface WindowUtils {
     ...
     * @param windowClass 字符串: 窗口类名，如果为空，则匹配所有. 这里的匹配是模糊匹配.
     * @param windowTitle 字符串: 窗口标题,如果为空，则匹配所有.这里的匹配是模糊匹配.
     ...
     * @param hwnd 整形数: 指定的窗口句柄. 如果为0,则对当前激活的窗口发送.
     ...
     * 函数简介:设置窗口的透明度
     * @param hwnd 整形数: 指定的窗口句柄
     * @param trans 整形数: 透明度取值(0-255) 越小透明度越大 0为完全透明(不可见) 255为完全显示(不透明)
     */
