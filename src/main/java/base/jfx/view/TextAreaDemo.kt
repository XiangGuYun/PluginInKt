...
class TextAreaDemo : BaseApp() {
    ...
    override fun init(window: Window) {
        val ta = TA().apply {
            ...
            //限制输入类型
            textFormatter = TextFormatter<String>(UnaryOperator<TextFormatter.Change?> { t ->
                if(t!!.text.matches(Regex("[a-z]*")))
                    t
                else
                    null
            })
        ...
        //鸡肋功能
//        ta.textFormatter = TextFormatter<String>(object : StringConverter<String>(){
...
//                string?.pln()
//                return if(string != null) return string.replace("5","五") else ""
//            }
//        })
//        ta.commitValue()
        window.scene = Scene(VBox().addChildren(
                ...
                HBox().addChildren(
                        ...
                        BN("自动翻页").clickBN {
                            ...
                            ta.scrollTopProperty().addListener { observable, oldValue, newValue ->
                                "$oldValue $newValue".pln()
                            }
                        }
                )
        ))
    ...
    companion object {
        ...
        fun main(args: Array<String>) {
            launch(TextAreaDemo::class.java)
        }
    }

    var sanguo = "话说天下大势，分久必合，合久必分。周末七国分争，并入于秦。及秦灭之后，楚、汉\n" +
